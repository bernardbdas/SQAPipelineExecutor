...
public class SQAPipelineExecutor extends Builder implements SimpleBuildStep {
	...
	public void perform(Run<?, ?> run, FilePath workspace, EnvVars env, Launcher launcher, TaskListener listener)
		...
		ExecutionImpl exec_obj = new ExecutionImpl(this.exec_token, this.app_url.toLowerCase(), this.threshold,
				this.verbose, listener);
			...
			while ((exec_dto.checkExecStatus().equalsIgnoreCase("INPROGRESS"))
				...
				if (executed < exec_obj.getExecutedTcs()) {
					...
					exec_dto.printLog("\n" + " ".repeat(27) + "(Threshold: " + exec_obj.getThreshold() + " % i.e. "
							+ ((exec_obj.getThreshold() / 100.00) * Double.valueOf(exec_obj.getTotalTcs()).intValue()
	...
	public static final class DescriptorImpl extends BuildStepDescriptor<Builder> {
		...
		public FormValidation doCheckExec_token(@QueryParameter String value) throws IOException, ServletException {
			...
			validationList
					.add(FormValidation.error(Messages.SQAPipelineExecutor_DescriptorImpl_errors_emptyExecToken()));
